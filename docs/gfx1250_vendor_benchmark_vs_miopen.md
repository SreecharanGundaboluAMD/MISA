...
## Correction (2026-08-30): every `_direct` (direct_store) number above was measuring the wrong kernel

Found while hardware-validating `docs/gfx1250_optimization_backlog.md`'s P2 item: the
driver's C++ kernel-name builder (`driver/igemm_gtc_base.h`) never gained a `_direct`
suffix when Phase 59 added `direct_store` (it was added to Python's codegen naming
only). So every `_direct`-suffixed result in the "Update (2026-08-28): fresh benchmark
after Phase 59" table above was actually `hipModuleGetFunction` silently resolving to
a same-shaped **non**-direct-store sibling kernel that happened to share the same
(un-suffixed) name — the LDS-reshuffle epilogue was still running, not the direct-store
one. Fixed (see the backlog's P2 entry for the fix). **The "direct_store wins on 8/17
fwd shapes, small bwd wins" narrative in the table above is unverified and should not
be trusted until this benchmark is re-run** with the fix in place — hardware-validated
correctness (`valid:y`) is confirmed post-fix, but the *performance* comparison that
produced this section's numbers needs to be redone from scratch.
