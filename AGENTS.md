...
| `docs/gfx1250_fp32_tdm_nan_regression.md` | **Known issue, not bisected.** fp32 + `tdm_global_load` produces silent `-nan` output on fwd and bwd, on every shape tested — including the exact shape a prior phase recorded as hardware-validated, so this is a regression, not an unvalidated gap. fp16/bf16 TDM are unaffected. See Known Issues below. |
...
- **fp32 + `tdm_global_load` produces `-nan` (regression, not bisected).** Every `tdm_global_load=1` fp32 kernel (fwd and bwd) produces silent `-nan` output on every shape tested, including the exact shape `docs/gfx1250_wmma_layout.md`'s Phase 29 recorded as hardware-validated `valid:y` — this is a regression sometime after that phase, not a never-validated path. fp16/bf16 TDM are unaffected (isolates the bug to fp32 specifically). Not bisected, not fixed, no rejection guard added (no fp32 TDM config is in wide use today). See `docs/gfx1250_fp32_tdm_nan_regression.md`. Separately, `config/igemm_wrw_gtc_gfx1250_nhwc_fp32_all.config` fails to build entirely (unrelated kernel-name collision, same doc) — the wrw fp32 master is currently unusable regardless of the TDM bug.
