...
| `docs/gfx1250_fp32_tdm_nan_regression.md` | **Fixed, root-caused (`3d8f3ea`).** fp32 + `tdm_global_load` produced silent `-nan` output — bisected to `16bbbfa` (COR-001 forced `lds_double_buffer=1` for fp32, but TDM's tensor descriptor LDS base wasn't toggled by `emit_buffer_switch()`). Fixed by adding `buffer_switch_extra_functor` to toggle TDM descriptor SGPRs in lockstep with VGPR offsets. See Known Issues below. |
...
- **~~fp32 + `tdm_global_load` produces `-nan`~~** **FIXED, root-caused (`3d8f3ea`).** Bisected to `16bbbfa` (COR-001 enforced `lds_double_buffer=1` for fp32, but TDM's tensor descriptor LDS base in SGPRs wasn't toggled by `emit_buffer_switch()`). Fixed by adding `buffer_switch_extra_functor` to toggle TDM descriptor SGPRs in lockstep with VGPR offsets. Hardware-validated: fwd/bwd `_tdm`/`_tdm_direct`, 3 shapes each, all `valid:y`. See `docs/gfx1250_fp32_tdm_nan_regression.md`. The separate wrw fp32 master build collision (dedup key-order bug in `script/build_gfx1250_master_configs.py`'s `normalize()` failing to recognize two semantically-identical sections with swapped `gemm_k_global_split`/`lds_double_buffer` line order as duplicates) is also now **FIXED** — `normalize()` canonicalizes by key-sorted representation; wrw fp32 master builds cleanly and is hardware-validated `valid:y`.
